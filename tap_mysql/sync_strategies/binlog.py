    ...
    with connect_with_backoff(mysql_conn) as open_conn:
        with open_conn.cursor() as cur:
            ...
            if binlog_format != "ROW":
                raise Exception(
                    "Unable to replicate binlog stream because binlog_format is not set to 'ROW': {}.".format(
                        binlog_format
                    )
                )
            ...
            if binlog_row_image != "FULL":
                raise Exception(
                    "Unable to replicate binlog stream because binlog_row_image is not set to 'FULL': {}.".format(
                        binlog_row_image
                    )
                )
    ...
    with connect_with_backoff(mysql_conn) as open_conn:
        with open_conn.cursor() as cur:
            ...
            if not existing_log_file:
                raise Exception(
                    "Unable to replicate binlog stream because log file {} does not exist.".format(
                        log_file
                    )
                )
            ...
            if log_pos > current_log_pos:
                raise Exception(
                    "Unable to replicate binlog stream because requested position ({}) for log file {} is greater than current position ({}).".format(
                        log_pos, log_file, current_log_pos
                    )
                )
...
def generate_streams_map(binlog_streams):
    ...
    for catalog_entry in binlog_streams:
        columns = add_automatic_properties(
            catalog_entry, list(catalog_entry.schema.properties.keys())
        )
    ...
    for binlog_event in reader:
        if isinstance(binlog_event, RotateEvent):
            state = update_bookmarks(
                state, binlog_streams_map, binlog_event.next_binlog, binlog_event.position
            )
        else:
